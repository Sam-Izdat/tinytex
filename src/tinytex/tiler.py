...
class Tiler:
    ...
    #.............
    @classmethod
    def get_tile_position(cls, idx:int, rows:int, cols:int) -> (int, int):
        """
    ...
    @classmethod
    def merge(cls, tiles:torch.Tensor, num_rows:int, num_cols:int) -> torch.Tensor:
        """
        ...
        :param num_rows: total number of rows in tile grid
        :param num_cols: total number of columns in tile grid
        :return: combined image tensor sized [C, H, W]
        """
        assert len(tiles.size()) == 4, "image tensor must be sized [N, C, H, W]"
        N, C, H, W = tiles.size()      
        assert num_rows * num_cols == N, "number of tiles must match rows * columns"
        combined_im = torch.zeros((1, C, num_rows * H, num_cols * W))
    ...
    @classmethod
    def blend(cls, tiles:torch.Tensor, num_rows:int=1, num_cols:int=1, wrap=True) -> torch.Tensor:
        ndim = len(im.size())
        assert ndim == 3 or ndim == 4, "tensor must be sized [C, H, W] or [N, C, H, W]"
        ...
        if N == 1:
            ...
            for n in range(N):
                pos_r, pos_c = cls.get_tile_position(n, num_rows, num_cols)
                t, r, b, l = cls.get_tile_neighbors(pos_r, pos_c, num_rows, num_cols, wrap=wrap)
                for c in range(C):
